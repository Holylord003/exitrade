...
exports.socialDeclined = async (email,reason) => {

    //GET Site Email
    const {website_title, website_email} = await getWebsiteSettings()
    //Get The Template
    const html = await ejs.renderFile(path.join(process.cwd(),"email","templates","sDeclined.ejs"),{reason});

    const mailOptions = {
    from: `${website_title} <${website_email}>`,
    to: email,
    subject: 'Your Social Media Followers Campaign Got Disapproved!',
    html
    };
